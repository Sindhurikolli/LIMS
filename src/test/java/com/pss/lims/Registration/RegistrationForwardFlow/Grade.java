...
public class Grade extends RegistrationLoginDetails {
	...
	public void createGrade() throws Exception {
		...
		output = System.getProperty("user.dir") + "\\" + "/TestReport/" + "CreateGrade" + (new Random().nextInt())
				+ ".pdf";
		fos = new FileOutputStream(output);
		writer = PdfWriter.getInstance(document, fos);
		writer.setStrictImageSequence(true);
		writer.open();
		HeaderFooterPageEvent event = new HeaderFooterPageEvent("Create Grade", "LIMS-SM-028", "Pass");
		...
		wait.until(ExpectedConditions.elementToBeClickable(By.cssSelector("a[href='gradeRegnPage.do'")));
		JavascriptExecutor jse1 = (JavascriptExecutor) driver;
		WebElement element1 = driver.findElement(By.cssSelector("a[href='gradeRegnPage.do'"));
		jse1.executeScript("arguments[0].scrollIntoView(true);", element1);
		Thread.sleep(1000);
		jse1.executeScript("arguments[0].click();", element1);
		document = Utilities.getScreenShotAndAddInLogDoc(driver, document, "Click on GRADE REGISTRATION", sno, false);
		Thread.sleep(4000);
		methodTocreateGrade();
		document.close();
		writer.close();
	...
	private void methodTocreateGrade() throws Exception {
...
//		((JavascriptExecutor) driver).executeScript("document.body.style.zoom='90%';");
		...
		sno++;
		JavascriptExecutor jse1 = (JavascriptExecutor) driver;
		WebElement element1 = driver.findElement(By.id("createGradeAction"));
		jse1.executeScript("arguments[0].click();", element1);
		document = Utilities.getScreenShotAndAddInLogDoc(driver, document, "Click on Registration of Grade", sno,
				false);
		Thread.sleep(3000);
		sno++;
		...
		Thread.sleep(5000);
		sno++;
		driver.findElement(By.id("gradeNameInLims")).sendKeys(properties.getProperty("Grade_Name"));
		document = Utilities.getScreenShotAndAddInLogDoc(driver, document, "Enter Name", sno, false);
		Thread.sleep(2000);
		sno++;
		driver.findElement(By.name("gradeDescriptionInLims")).sendKeys(properties.getProperty("Grade_Description"));
		document = Utilities.getScreenShotAndAddInLogDoc(driver, document, "Enter Description", sno, false);
		Thread.sleep(2000);
		sno++;
		JavascriptExecutor jse3 = (JavascriptExecutor) driver;
		WebElement element3 = driver.findElement(By.id("selAppFromUserInGradeReg"));
