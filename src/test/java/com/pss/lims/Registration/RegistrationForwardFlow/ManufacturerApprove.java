...
public class ManufacturerApprove extends RegistrationLoginDetails {
	...
	public void approveManufactruer() throws Exception {
		...
		output = System.getProperty("user.dir") + "\\" + "/TestReport/" + "ApproveManufacturer"
				+ (new Random().nextInt()) + ".pdf";
		fos = new FileOutputStream(output);
		writer = PdfWriter.getInstance(document, fos);
		writer.setStrictImageSequence(true);
		writer.open();
		HeaderFooterPageEvent event = new HeaderFooterPageEvent("Approve Manufacturer", "LIMS-SM-046", "Pass");
	...
	private void methodToapproveManufactruer() throws Exception {
		...
		if (isRecordSelected) {
			...
			Thread.sleep(2000);
			sno++;
			driver.findElement(By.id("apprBtnInManufactApprovalWindow")).click();
		...
		if (perPageNoOfRecordsPresent > 0) {
			while (noOfRecordsChecked < totalNoOfRecords) {
				if (totalNoOfRecords > 1) {
					...
					if (manufacturerName.equalsIgnoreCase(manufacturerNameSequence)) {
						driver.findElement(
								By.xpath("//*[@id=\"manufacturerApprovalTable\"]/div/table/tbody/tr/td[3]"))
								.click();
						isRecordSelected = true;
						break;
					}
				...
				if ((!isRecordSelected) && (noOfRecordsChecked < totalNoOfRecords)) {
					...
					Thread.sleep(4000);
					table = driver.findElement(By.id("manufacturerApprovalTable"));// Document Tree approve table
