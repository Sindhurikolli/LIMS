...
public class ApproveUOM extends RegistrationLoginDetails {
	...
	public void approveUOM() throws Exception {
		...
		output = System.getProperty("user.dir") + "\\" + "/TestReport/" + "ApproveUOM" + (new Random().nextInt())
				+ ".pdf";
		fos = new FileOutputStream(output);
		writer = PdfWriter.getInstance(document, fos);
		writer.setStrictImageSequence(true);
		writer.open();
		HeaderFooterPageEvent event = new HeaderFooterPageEvent("Approve UOM", "PSS-LIMS-008", "Pass");
...
//			Thread.sleep(2000);
//			sno++;
			driver.findElement(By.id("uomCommentsInApproval")).sendKeys(properties.getProperty("Approval_Comments"));
			...
			while (noOfRecordsChecked < totalNoOfRecords) {
				if (totalNoOfRecords > 1) {
					for (int i = 1; i <= perPageNoOfRecordsPresent; i++) {
						...
						if (uom.equalsIgnoreCase(DevNumberSequence)) {
							driver.findElement(By.xpath(
									"//*[@id=\"uomTableInApproval\"]/div/table/tbody/tr[ " + i + " ]/td[3]"))
									.click();
					...
					String DevNumberSequence = driver
							...
							.getText();
					if (uom.equalsIgnoreCase(DevNumberSequence)) {
						driver.findElement(By.xpath("//*[@id=\"uomTableInApproval\"]/div/table/tbody/tr/td[3]"))
								.click();
						isRecordSelected = true;
						break;
					}
