...
public class GroupType extends RegistrationLoginDetails {
	...
	public void createGroupType() throws Exception {
		...
		output = System.getProperty("user.dir") + "\\" + "/TestReport/" + "CreateGroupType" + (new Random().nextInt())
				+ ".pdf";
		fos = new FileOutputStream(output);
		writer = PdfWriter.getInstance(document, fos);
		writer.setStrictImageSequence(true);
		writer.open();
		HeaderFooterPageEvent event = new HeaderFooterPageEvent("Create Group Type", "LIMS-SM-033", "Pass");
	...
	private void methodToCreateGroupType() throws Exception {
...
//		((JavascriptExecutor) driver).executeScript("document.body.style.zoom='90%';");
		...
		sno++;
		JavascriptExecutor jse1 = (JavascriptExecutor) driver;
		WebElement element1 = driver.findElement(By.id("createGroupTypeAction"));
		jse1.executeScript("arguments[0].click();", element1);
		document = Utilities.getScreenShotAndAddInLogDoc(driver, document, "Click on Registration of Group Type", sno,
				false);
		Thread.sleep(3000);
		sno++;
		...
		WebElement element2 = driver.findElement(By.id("selAppFromUserInGroupTypeReg"));
