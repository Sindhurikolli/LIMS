...
public class GroupRegistration extends RegistrationLoginDetails {

	@Test
	public void createGroupRegistration() throws Exception {

		document = new Document();
		Font font = new Font(Font.FontFamily.TIMES_ROMAN);
		output = System.getProperty("user.dir") + "\\" + "/TestReport/" + "CreateGroupRegistration"
				+ (new Random().nextInt()) + ".pdf";
		fos = new FileOutputStream(output);
		writer = PdfWriter.getInstance(document, fos);
		writer.setStrictImageSequence(true);
		writer.open();
		HeaderFooterPageEvent event = new HeaderFooterPageEvent("Create Group Registration", "LIMS-SM-037", "Pass");
	...
	private void methodToCreateGroupReg() throws Exception {
...
//		((JavascriptExecutor) driver).executeScript("document.body.style.zoom='90%';");
		...
		sno++;
		JavascriptExecutor jse1 = (JavascriptExecutor) driver;
		WebElement element1 = driver.findElement(By.id("createGroupAction"));
		jse1.executeScript("arguments[0].click();", element1);
		document = Utilities.getScreenShotAndAddInLogDoc(driver, document, "Click on Registration of Group", sno,
				false);
		Thread.sleep(3000);
		sno++;
		...
		Thread.sleep(2000);
		sno++;
		Select groupType = new Select(driver.findElement(By.id("groupTypeInLimsGroup")));
		Thread.sleep(2000);
		groupType.selectByVisibleText(properties.getProperty("Group_Type"));
		document = Utilities.getScreenShotAndAddInLogDoc(driver, document, "Select Group Type", sno, false);
		Thread.sleep(2000);
		sno++;
		JavascriptExecutor jse21 = (JavascriptExecutor) driver;
		WebElement element21 = driver.findElement(By.id("selAppFromUserInGroupReg"));
		...
		if (isRecordSelected) {
			...
			Thread.sleep(3000);
			sno++;
			driver.findElement(By.id("groupDescriptionInLims")).sendKeys(properties.getProperty("Group_Description"));
