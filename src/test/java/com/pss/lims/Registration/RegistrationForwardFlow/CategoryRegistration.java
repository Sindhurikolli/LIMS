...
public class CategoryRegistration extends RegistrationLoginDetails {

	@Test
	public void createCategoryRegistration() throws Exception {

		document = new Document();
		Font font = new Font(Font.FontFamily.TIMES_ROMAN);
		output = System.getProperty("user.dir") + "\\" + "/TestReport/" + "CreateCategory" + (new Random().nextInt())
				+ ".pdf";
		fos = new FileOutputStream(output);
		writer = PdfWriter.getInstance(document, fos);
		writer.setStrictImageSequence(true);
		writer.open();
		HeaderFooterPageEvent event = new HeaderFooterPageEvent("Create Category", "LIMS-SM-020", "Pass");
		...
		document = Utilities.getScreenShotAndAddInLogDoc(driver, document, "Click on CATEGORY REGISTRATION", sno,
				false);
		Thread.sleep(4000);
		methodTocreateCategoryRegistration();
		document.close();
		writer.close();
	...
	private void methodTocreateCategoryRegistration() throws Exception {
...
//		((JavascriptExecutor) driver).executeScript("document.body.style.zoom='90%';");
		...
		sno++;
		JavascriptExecutor jse1 = (JavascriptExecutor) driver;
		WebElement element1 = driver.findElement(By.id("createCategoryRegAction"));
		jse1.executeScript("arguments[0].click();", element1);
		document = Utilities.getScreenShotAndAddInLogDoc(driver, document, "Click on Registration of Category", sno,
				false);
		Thread.sleep(3000);
		sno++;
		...
		driver.findElement(By.id("categoryRegDescriptionInLims"))
				.sendKeys(properties.getProperty("Category_Description"));
		document = Utilities.getScreenShotAndAddInLogDoc(driver, document, "Enter Description", sno, false);
		Thread.sleep(2000);
		sno++;
		driver.findElement(By.id("uniqCodeLimsCategoryReg")).sendKeys(properties.getProperty("Category_Unique_Code"));
		document = Utilities.getScreenShotAndAddInLogDoc(driver, document, "Enter Unique Code", sno, false);
		Thread.sleep(2000);
		sno++;
		JavascriptExecutor jse3 = (JavascriptExecutor) driver;
		WebElement element3 = driver.findElement(By.id("selAppFromUserInCategoryReg"));
