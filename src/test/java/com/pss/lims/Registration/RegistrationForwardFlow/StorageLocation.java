...
public class StorageLocation extends RegistrationLoginDetails {
	...
	public void createStorageLocation() throws Exception {
		...
		output = System.getProperty("user.dir") + "\\" + "/TestReport/" + "CreateStorageLocation"
				+ (new Random().nextInt()) + ".pdf";
		fos = new FileOutputStream(output);
		writer = PdfWriter.getInstance(document, fos);
		writer.setStrictImageSequence(true);
		writer.open();
		HeaderFooterPageEvent event = new HeaderFooterPageEvent("Create Storage Location", "LIMS-SM-013", "Pass");
		...
		wait.until(ExpectedConditions.elementToBeClickable(By.cssSelector("a[href='storageLocations.do'")));
		JavascriptExecutor jse1 = (JavascriptExecutor) driver;
		WebElement element1 = driver.findElement(By.cssSelector("a[href='storageLocations.do'"));
		jse1.executeScript("arguments[0].scrollIntoView(true);", element1);
		Thread.sleep(1000);
		jse1.executeScript("arguments[0].click();", element1);
		document = Utilities.getScreenShotAndAddInLogDoc(driver, document, "Click on Storage Location", sno, false);
		Thread.sleep(4000);
		methodTocreateStorageLocation();
		document.close();
		writer.close();
	...
	private void methodTocreateStorageLocation() throws Exception {

		Thread.sleep(3000);
		driver.findElement(By.id("createStorageLocnAction")).click();
		document = Utilities.getScreenShotAndAddInLogDoc(driver, document, "Click on Radio Button", sno, false);
		Thread.sleep(3000);
		...
		Thread.sleep(5000);
		sno++;
		driver.findElement(By.id("strgLocInStorageLocation")).sendKeys(properties.getProperty("Storage_Location_Name"));
		document = Utilities.getScreenShotAndAddInLogDoc(driver, document, "Enter Storage Location", sno, false);
		Thread.sleep(2000);
		sno++;
		driver.findElement(By.id("descInStorageLocation"))
				.sendKeys(properties.getProperty("Storage_Location_Description"));
		document = Utilities.getScreenShotAndAddInLogDoc(driver, document, "Enter Description", sno, false);
		Thread.sleep(2000);
		sno++;
		JavascriptExecutor jse3 = (JavascriptExecutor) driver;
		WebElement element3 = driver.findElement(By.id("selAppFromUserInStorageLocation"));
