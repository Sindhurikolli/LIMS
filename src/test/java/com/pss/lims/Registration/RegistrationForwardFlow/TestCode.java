...
public class TestCode extends RegistrationLoginDetails {
	...
	public void createTestCode() throws Exception {
		...
		output = System.getProperty("user.dir") + "\\" + "/TestReport/" + "CreateTestCode" + (new Random().nextInt())
				+ ".pdf";
		fos = new FileOutputStream(output);
		writer = PdfWriter.getInstance(document, fos);
		writer.setStrictImageSequence(true);
		writer.open();
		HeaderFooterPageEvent event = new HeaderFooterPageEvent("Create Test Code", "LIMS-SM-024", "Pass");
		...
		wait.until(ExpectedConditions.elementToBeClickable(By.cssSelector("a[href='testCodeRegnPage.do'")));
		JavascriptExecutor jse1 = (JavascriptExecutor) driver;
		WebElement element1 = driver.findElement(By.cssSelector("a[href='testCodeRegnPage.do'"));
		jse1.executeScript("arguments[0].scrollIntoView(true);", element1);
		Thread.sleep(1000);
		jse1.executeScript("arguments[0].click();", element1);
		document = Utilities.getScreenShotAndAddInLogDoc(driver, document, "Click on Test Code", sno, false);
		Thread.sleep(4000);
		methodTocreateTestCode();
		document.close();
		writer.close();
	...
	private void methodTocreateTestCode() throws Exception {
...
//		((JavascriptExecutor) driver).executeScript("document.body.style.zoom='90%';");
		...
		sno++;
		JavascriptExecutor jse1 = (JavascriptExecutor) driver;
		WebElement element1 = driver.findElement(By.id("createTestCodeAction"));
		jse1.executeScript("arguments[0].click();", element1);
		document = Utilities.getScreenShotAndAddInLogDoc(driver, document, "Click on Registration of Test Code", sno,
				false);
		Thread.sleep(3000);
		sno++;
		...
		Thread.sleep(5000);
		sno++;
		driver.findElement(By.id("testCodeInCategoryInLims")).sendKeys(properties.getProperty("Category_TestCode"));
		document = Utilities.getScreenShotAndAddInLogDoc(driver, document, "Enter Test Code", sno, false);
		Thread.sleep(2000);
		sno++;
		Select testCategory = new Select(driver.findElement(By.id("testCategoryInCatInLims")));
		Thread.sleep(2000);
		testCategory.selectByVisibleText(properties.getProperty("Category_Name"));
		document = Utilities.getScreenShotAndAddInLogDoc(driver, document, "Select Test Catgeory", sno, false);
		Thread.sleep(2000);
		sno++;
		JavascriptExecutor jse3 = (JavascriptExecutor) driver;
		WebElement element3 = driver.findElement(By.id("selAppFromUserInTestCodeReg"));
