...
public class UOM extends RegistrationLoginDetails {
	...
	public void createUOM() throws Exception {
		...
		output = System.getProperty("user.dir") + "\\" + "/TestReport/" + "CreateUOM" + (new Random().nextInt())
				+ ".pdf";
		fos = new FileOutputStream(output);
		writer = PdfWriter.getInstance(document, fos);
		writer.setStrictImageSequence(true);
		writer.open();
		HeaderFooterPageEvent event = new HeaderFooterPageEvent("Create UOM", "LIMS-SM-005", "Pass");
	...
	private void methodTocreateUOM() throws Exception {
...
//		((JavascriptExecutor) driver).executeScript("document.body.style.zoom='90%';");
		...
		sno++;
		JavascriptExecutor jse1 = (JavascriptExecutor) driver;
		WebElement element1 = driver.findElement(By.id("createUomAction"));
		jse1.executeScript("arguments[0].click();", element1);
		document = Utilities.getScreenShotAndAddInLogDoc(driver, document, "Click on registration of UOM", sno, false);
		Thread.sleep(3000);
		sno++;
		...
		Thread.sleep(2000);
		sno++;
		driver.findElement(By.id("descInUomRegn")).sendKeys(properties.getProperty("UOM_Description"));
		document = Utilities.getScreenShotAndAddInLogDoc(driver, document, "Enter Description", sno, false);
		Thread.sleep(3000);
		sno++;
		JavascriptExecutor jse3 = (JavascriptExecutor) driver;
		WebElement element3 = driver.findElement(By.id("selAppFromUserInUomRegn"));
