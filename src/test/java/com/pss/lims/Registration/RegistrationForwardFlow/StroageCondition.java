...
public class StroageCondition extends RegistrationLoginDetails {
	...
	public void createStorageCondition() throws Exception {
		...
		output = System.getProperty("user.dir") + "\\" + "/TestReport/" + "CreateStorageCondition"
				+ (new Random().nextInt()) + ".pdf";
		fos = new FileOutputStream(output);
		writer = PdfWriter.getInstance(document, fos);
		writer.setStrictImageSequence(true);
		writer.open();
		HeaderFooterPageEvent event = new HeaderFooterPageEvent("Create Storage Condition", "LIMS-SM-009", "Pass");
	...
	private void methodTocreateStorageCondition() throws Exception {
		...
		document = Utilities.getScreenShotAndAddInLogDoc(driver, document, "Click on Create Radio Button", sno, false);
		//Thread.sleep(3000);
		Thread.sleep(3000);
		...
		document = Utilities.getScreenShotAndAddInLogDoc(driver, document, "Enter Storage Condition", sno, false);
		Thread.sleep(2000);
		sno++;
		driver.findElement(By.id("descInStorageCondition"))
				.sendKeys(properties.getProperty("Storage_Condition_Description"));
		document = Utilities.getScreenShotAndAddInLogDoc(driver, document, "Enter Description", sno, false);
		Thread.sleep(2000);
		sno++;
		JavascriptExecutor jse3 = (JavascriptExecutor) driver;
		WebElement element3 = driver.findElement(By.id("selAppFromUserInStorageCondition"));
