...
public class ApproveProtocol extends LoginDetails {

	@Test
	public void approveProtocol() throws Exception {

		document = new Document();
		Font font = new Font(Font.FontFamily.TIMES_ROMAN);
		output = System.getProperty("user.dir") + "\\" + "/TestReport/" + "ApproveProtocol" + (new Random().nextInt())
				+ ".pdf";
		fos = new FileOutputStream(output);
		writer = PdfWriter.getInstance(document, fos);
		writer.setStrictImageSequence(true);
		writer.open();
		HeaderFooterPageEvent event = new HeaderFooterPageEvent("Approve Protocol", "LIMS-SM-005", "Pass");
		...
		wiat.until(ExpectedConditions.elementToBeClickable(By.cssSelector("a[href='protocolApprovalStabReq.do'")));
		driver.findElement(By.cssSelector("a[href='protocolApprovalStabReq.do'")).click();
		document = Utilities.getScreenShotAndAddInLogDoc(driver, document, "Click on Protocol", sno, false);
		wiat.until(ExpectedConditions.presenceOfElementLocated(By
				.cssSelector("#protocolStabReqApprovalTable > div > div.jtable-busy-message[style='display: none;']")));
		Thread.sleep(4000);
		methodToApproveProtocol();
		document.close();
		writer.close();
	...
	private void methodToApproveProtocol() throws Exception {
		...
		if (isRecordSelected) {
			...
			document = Utilities.getScreenShotAndAddInLogDoc(driver, document,
					"Select a Record And Click on View & Approval", sno, false);
			Thread.sleep(5000);
			JavascriptExecutor jse1 = (JavascriptExecutor) driver;
			WebElement element1 = driver.findElement(By.id("approveBtnInProtocolStabApproval"));
			jse1.executeScript("arguments[0].scrollIntoView(true);", element1);
			Thread.sleep(2000);
			sno++;
			driver.findElement(By.id("commentsInProtAppForm")).sendKeys(properties.getProperty("Approval_Comments"));
			document = Utilities.getScreenShotAndAddInLogDoc(driver, document, "Enter Comments", sno, false);
			Thread.sleep(3000);
			sno++;
			driver.findElement(By.id("approveBtnInProtocolStabApproval")).click();
	...
	private boolean selectRecordForProtocol(int count, boolean isRecordSelected, String name) throws Exception {
		...
		WebElement table = driver.findElement(By.id("protocolStabReqApprovalTable"));
		WebElement tableBody = table.findElement(By.tagName("tbody"));
		int perPageNoOfRecordsPresent = tableBody.findElements(By.tagName("tr")).size();
		int totalNoOfRecords = 0;
		int noOfRecordsChecked = 0;
		if (perPageNoOfRecordsPresent > 0) {
			String a = driver.findElement(By.xpath("//*[@id=\"protocolStabReqApprovalTable\"]/div/div[4]/div[2]/span"))
					.getText();// For
			// Ex:
			// Showing
			// 1-1
		...
		if (perPageNoOfRecordsPresent > 0 && count == 0) {
			if ((totalNoOfRecords > 1) && ((name == null) || ("".equalsIgnoreCase(name)))) {
				name = driver
						.findElement(By.xpath("//*[@id=\"protocolStabReqApprovalTable\"]/div/table/tbody/tr[1]/td[2]"))
						.getText();// documentType
			} else if ((name == null) || ("".equalsIgnoreCase(name))) {
				name = driver
						.findElement(By.xpath("//*[@id=\"protocolStabReqApprovalTable\"]/div/table/tbody/tr/td[2]"))
						.getText();// document
									// type
			}
			++count;
				...
				if (totalNoOfRecords > 1) {
					for (int i = 1; i <= perPageNoOfRecordsPresent; i++) {
						String DevNumberSequence = driver.findElement(By.xpath(
								"//*[@id=\"protocolStabReqApprovalTable\"]/div/table/tbody/tr[ " + i + " ]/td[2]"))
