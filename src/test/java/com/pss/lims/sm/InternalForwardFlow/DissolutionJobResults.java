...
public class DissolutionJobResults extends SMLoginDetails {
	...
	private void methodToapproveJobResults() throws Exception {
		...
		if (isRecordSelected) {
			...
			Thread.sleep(3000);
			Helper.waitLoadRecords(driver, By.id("noOfVarSetsInJobResLimsDissolutionTest"));
			String sets =driver.findElement(By.id("noOfVarSetsInJobResLimsDissolutionTest")).getText();
			//int noofSets=Integer.parseInt(sets.trim());
			int noofSets=6;
			for(int j=1;j<=noofSets;j++ ) {
				
				int noofVariables = Integer.parseInt(properties.getProperty("No_of_variables_DT"));
				String[] Variables = properties.getProperty("Verify_Value_Dissolution").split(",");
...
//				Thread.sleep(2000);
				driver.findElement(By
						.xpath("//*[@id='variableGridStoreInDissolutionJobResult" + j + "']/div/table/tbody/tr[" + i + "]/td[4]/input"))
						.sendKeys(Variables[i - 1]);
				Thread.sleep(2000);
			}
			}//*[@id="option3_InQualJobResult1"]
...
//			Thread.sleep(3000);
			sno++;
			driver.findElement(By.id("submitBtnInDissTestInJobResults")).click();
		...
		if (perPageNoOfRecordsPresent > 0) {
			while (noOfRecordsChecked < totalNoOfRecords) {
				...
				if ((!isRecordSelected) && (noOfRecordsChecked < totalNoOfRecords)) {
					Helper.scrollAndClickElement(driver, By.cssSelector("#jobResultsJTable > div > div.jtable-bottom-panel > div.jtable-left-area > span.jtable-page-list > span.jtable-page-number-next"));
