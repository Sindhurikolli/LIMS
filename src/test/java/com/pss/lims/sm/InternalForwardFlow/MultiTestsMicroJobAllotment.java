...
public class MultiTestsMicroJobAllotment extends SMLoginDetails {

	@Test
	public void createMicroJobAllotment() throws Exception {

		document = new Document();
		Font font = new Font(Font.FontFamily.TIMES_ROMAN);
		output = System.getProperty("user.dir") + "\\" + "/TestReport/" + "JobAllotment" + (new Random().nextInt())
				+ ".pdf";
		fos = new FileOutputStream(output);
		writer = PdfWriter.getInstance(document, fos);
		writer.setStrictImageSequence(true);
		writer.open();
		HeaderFooterPageEvent event = new HeaderFooterPageEvent("Job Allotment", "LIMS-SM-095", "Pass");
		...
		wait.until(ExpectedConditions.elementToBeClickable(By.cssSelector("a[href='microJobAllotmentInSample.do']")));
		driver.findElement(By.cssSelector("a[href='microJobAllotmentInSample.do']")).click();
		document = Utilities.getScreenShotAndAddInLogDoc(driver, document, "Click on JobAllotment", sno, false);
		wait.until(ExpectedConditions.presenceOfElementLocated(
				By.cssSelector("#limsMicroJobAllotmentGrid > div > div.jtable-busy-message[style='display: none;']")));
		Thread.sleep(4000);
		methodTocreateMicroJobAllotment();
		document.close();
		writer.close();
	...
	private void methodTocreateMicroJobAllotment() throws Exception {
...
//			isRecordSelected1 = selectRecordForJobAllotment(count1, isRecordSelected1, arNumber);
			sno++;
			driver.findElement(By.id("viewAndAllotBtnInLimsMicroJobAllotment")).click();
			...
			Thread.sleep(2000);
			int rows=driver.findElements(By.xpath("//*[@id=\"TestGridInLimsJobAllotMent\"]/div/table/tbody/tr")).size();
			PropertiesConfiguration properties1= new PropertiesConfiguration(
					"src/test/java/LIMSUIProperties/SampleManagement.properties");
			properties1.setProperty("NoofTests", rows);
			properties1.save();
			for(int i=1; i<=rows; i++)
			{
				driver.findElement(By.xpath("//*[@id=\"TestGridInLimsJobAllotMent\"]/div/table/tbody/tr[" + i + "]/td[2]/input")).click();
			}
//			int count2 = 0;
//			boolean isRecordSelected2 = false;
//     		String name2 = properties.getProperty("Test_Types");
//			isRecordSelected2 = selectTest(count2, isRecordSelected2, name2);
			...
			Thread.sleep(5000);
			 try   
			  { 
				...
				if(driver.findElement(By.xpath("//*[@id=\"modal-window\"]/div/div/div[2]/font/b")).isDisplayed()) {
			  ...
			  catch(Exception e) {
				 {
					...
					System.out.println("This Is Not First Sample Of The Year");
				}	
			  }
			sno++;
			for(int j=1; j<=rows; j++)
			{
			driver.findElement(By.cssSelector("#selInchargeInTestAllot"+j)).click();
			...
			Thread.sleep(2000);
			sno++;
			Select user = new Select(driver.findElement(By.id("reviewerIdInTestAllot"+j)));
			Thread.sleep(2000);
...
//		}
		if (perPageNoOfRecordsPresent > 0 && count2 == 0) {
			if ((totalNoOfRecords > 1) && ((name2 == null) || ("".equalsIgnoreCase(name2)))) {
				name2 = driver
						.findElement(By.xpath("//*[@id=\"TestGridInLimsJobAllotMent\"]/div/table/tbody/tr/td[1]"))
						.getText();// documentType
			} else if ((name2 == null) || ("".equalsIgnoreCase(name2))) {
				name2 = driver.findElement(By.xpath("//*[@id=\"TestGridInLimsJobAllotMent\"]/div/table/tbody/tr/td[1]"))
						.getText();// document
									// type
			}
			++count2;
				...
				if (totalNoOfRecords > 1) {
					for (int i = 1; i <= perPageNoOfRecordsPresent; i++) {
						String arNumberSequence = driver.findElement(By
								.xpath("//*[@id=\"TestGridInLimsJobAllotMent\"]/div/table/tbody/tr[ " + i + " ]/td[1]"))
	...
	private boolean selectRecordForJobAllotment(int count1, boolean isRecordSelected1, String arNumber)
			throws Exception {
		// TODO Auto-generated method stub
		WebElement table = driver.findElement(By.id("limsMicroJobAllotmentGrid"));
		WebElement tableBody = table.findElement(By.tagName("tbody"));
		int perPageNoOfRecordsPresent = tableBody.findElements(By.tagName("tr")).size();
		int totalNoOfRecords = 0;
		int noOfRecordsChecked = 0;
		if (perPageNoOfRecordsPresent > 0) {
			String a = driver.findElement(By.xpath("//*[@id=\"limsMicroJobAllotmentGrid\"]/div/div[4]/div[2]/span")).getText();// For
			// Ex:
			// Showing
			// 1-1
			// of
			...
			while (noOfRecordsChecked < totalNoOfRecords) {
				if (totalNoOfRecords > 1) {
					for (int i = 1; i <= perPageNoOfRecordsPresent; i++) {
						String arNumberSequence = driver
								.findElement(By
										.xpath("//*[@id=\"limsMicroJobAllotmentGrid\"]/div/table/tbody/tr[ " + i + " ]/td[10]"))
								.getText();// documentTypeName
						if (arNumber.equalsIgnoreCase(arNumberSequence)) {
							driver.findElement(
									By.xpath("//*[@id=\"limsMicroJobAllotmentGrid\"]/div/table/tbody/tr[ " + i + " ]/td[10]"))
									.click();
				...
				if ((!isRecordSelected1) && (noOfRecordsChecked < totalNoOfRecords)) {
					//Helper.clickElement(driver, By.cssSelector("#limsMicroJobAllotmentGrid > div > div.jtable-bottom-panel > div.jtable-left-area > span.jtable-page-list > span.jtable-page-number-next"));
					driver.findElement(By.cssSelector(
							"#limsMicroJobAllotmentGrid > div > div.jtable-bottom-panel > div.jtable-left-area > span.jtable-page-list > span.jtable-page-number-next"))
					.click();// next page in Document approve list
					Thread.sleep(4000);
					table = driver.findElement(By.id("limsMicroJobAllotmentGrid"));// Document Tree approve table
					tableBody = table.findElement(By.tagName("tbody"));
					perPageNoOfRecordsPresent = tableBody.findElements(By.tagName("tr")).size();
				}
			}
