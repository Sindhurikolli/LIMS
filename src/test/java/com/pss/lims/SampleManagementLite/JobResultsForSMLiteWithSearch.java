...
public class JobResultsForSMLiteWithSearch extends SMLoginDetails {
	...
	public void JobResultsForSMLiteVersion() throws Exception {
		...
		output = System.getProperty("user.dir") + "\\" + "/TestReport/" + "JobResultsForSMLiteVersion"
				+ (new Random().nextInt()) + ".pdf";
		fos = new FileOutputStream(output);
		writer = PdfWriter.getInstance(document, fos);
		writer.setStrictImageSequence(true);
		writer.open();
		HeaderFooterPageEvent event = new HeaderFooterPageEvent("JobResultsForSMLiteVersion", "PSS-LIMS-097", "Pass");
